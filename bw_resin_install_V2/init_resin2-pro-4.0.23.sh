...
PKGS=$ROOT/pkgs
RESINVERSION=resin2_pro_4.0.23
...
ins_resin(){
	 ...
	 if [[ -f ${RESINVERSION}_build/install_done.tag ]];then
                echo "[$(datef)] ins_${RESINVERSION}(): ${RESINVERSION} installed, skin!"
                return
         fi
	...
	cd ../../../
    \cp -fv $SCRIPTS/license_resin_pro_4.0.23.jar /usr/local/${RESINVERSION}/lib/pro.jar
	\cp -fv $SCRIPTS/${RESINVERSION}.xml /usr/local/${RESINVERSION}/conf/${RESINVERSION}.xml

    \cp -fv $SCRIPTS/${RESINVERSION} /etc/init.d/
